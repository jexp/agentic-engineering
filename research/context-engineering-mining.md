...
**Source**: [`context-engineering-presentation.pdf`](./context-engineering-presentation.pdf), slides 1–40 (slides after 40 are research material, ignored).
...
1. **The Context Pyramid (Slide 18)** — Inverted pyramid: Persistent base / Dynamic middle / Ephemeral top + caching arrow. **Drops directly into [`workshop-cost-tokens.md`](../workshop/workshop-cost-tokens.md) Slide 1** as a much better visual than the table I drafted. Caching = stability ladder. Use this.
2. **5 Failure Modes of Context (Slide 12)** — Context Rot, Poisoning, Confusion, Distraction, Clash, with the Gemini-Pokemon and Gemini->100k examples and the dbreunig.com source. **Drops into [`workshop-practices.md`](../workshop/workshop-practices.md) Slide 2** (Challenges) — replaces my generic list with named, sourced phenomena.
3. **3-Phase Agent Loop with 3 Context Problem (Slides 22–23)** — Reason/Plan · Act/Execute · Observe/Process, with what each phase needs in context. **Drops into [`workshop-practices.md`](../workshop/workshop-practices.md) Slide 7** (per-task loop) as the conceptual frame *above* the 11-step loop.
4. **Context Structure principles (Slide 16)** — group, label (headers + XML), hierarchy, concise, refer back, reiterate at end (attention). **Drops into [`workshop-practices.md`](../workshop/workshop-practices.md) Slide 4** (AGENTS.md authoring) — these are the *why* under the format rules I gave.
5. **Karpathy "context engineering" tweet (Slide 5)** — pairs perfectly with the Feb 2025 "vibe coding" tweet already in [`workshop-intro.md`](../workshop/workshop-intro.md). **Add to intro Slide 3** (history) as the moment "context engineering" entered the vocabulary (June 2025).
...
|---|---|---|
| **5** | Karpathy "context engineering > prompt engineering" (June 2025 tweet) | [`workshop-intro.md`](../workshop/workshop-intro.md) Slide 3 — pair with Karpathy Feb 2025 vibe-coding tweet |
| **6** | "Why Context Matters Now" — Right info / Test Time > Train Time / Bigger ≠ better / Structured input → reliability | [`workshop-intro.md`](../workshop/workshop-intro.md) Slide 4 framing OR [`workshop-practices.md`](../workshop/workshop-practices.md) Slide 3 (latent space collapse) |
...
| **10** | Venn diagram of CE sources (RAG, Prompt Eng, State/History, Memory, Structured Outputs) — Dex Horthy / 12-Factor Agents | Optional: [`workshop-harnesses.md`](../workshop/workshop-harnesses.md) Slide 1 anatomy as an alternative visual |
| **11** | Sources of Context — Human / Cognitive / Memory / Data Tools (clean four-quadrant) | [`workshop-harnesses.md`](../workshop/workshop-harnesses.md) Slide 1 — or skip; my anatomy box is similar |
| **12** | **5 Failure Modes** — Rot / Poisoning / Confusion / Distraction / Clash with named examples | **[`workshop-practices.md`](../workshop/workshop-practices.md) Slide 2 — REPLACE my generic challenges list** |
| **13** | **MVC — Minimum Viable Context** — Clear instructions, Role/Goals/Constraints/Facts, Selected Examples, Minimal Tools, User Request. Aim: high signal-to-noise | **[`workshop-practices.md`](../workshop/workshop-practices.md) Slide 4 — frame AGENTS.md as MVC** |
...
| **16** | **Context Structure** — group, label clearly (headers + XML tags), hierarchy, concise, refer back, **reiterate at end** | **[`workshop-practices.md`](../workshop/workshop-practices.md) Slide 4 — AGENTS.md authoring rules** |
| **17** | **Approaches: Offload / Consolidate / Retrieve** — three ways to manage context | **[`workshop-cost-tokens.md`](../workshop/workshop-cost-tokens.md) Slide 2** — frame for the savings levers |
| **18** | **Context Pyramid** — Persistent base / Dynamic middle / Ephemeral top + caching arrow | **[`workshop-cost-tokens.md`](../workshop/workshop-cost-tokens.md) Slide 1 — REPLACE my "where the tokens go" graphic** |
| **19** | Lisa Maria Martin: "When we organize information, we change it" | quote bank — practices Slide 4 |
| **21** | **Lance Martin / LangChain — "Communication is all you need"**: "Most of the time when an agent is not performing reliably the underlying cause is not that the model is not intelligent enough, but rather that context and instructions have not been communicated properly to the model" | **Keynote!** This is the *single best quote* I've seen for the design-as-communication thread. Goes into [`vibecode-keynote-v3.md`](../keynote/vibecode-keynote-v3.md) and the workshop's mentoring frame. |
| **22** | **3-Phase Agent Loop** — Guidance / Thought / Action / Observation / Human-Agent | **[`workshop-practices.md`](../workshop/workshop-practices.md) Slide 7 — per-task loop frame** |
| **23** | **Dynamic Context for Agents — 3 Context Problem**: Reason/Plan (what to do?) · Act/Execute (how?) · Observe/Process (integrate results) | **[`workshop-practices.md`](../workshop/workshop-practices.md) Slide 7** as the *conceptual layer* above the 11-step loop. Each step in the 11-step loop maps to one of these three contexts. |
...
| **25** | Tools & Context — Push (RAG) vs Pull (Tool Calling), Tool Selection (API design), Tool outputs (structure/volume/relevancy/explainability) | [`workshop-harnesses.md`](../workshop/workshop-harnesses.md) Slide 3 (craft levers) — as a Tools subsection |
| **26** | Multi-Agent Systems — Divide & Conquer / Conflicting Decisions / Context Isolation | [`workshop-harnesses.md`](../workshop/workshop-harnesses.md) or [`workshop-practices.md`](../workshop/workshop-practices.md) aside on sub-agents |
...
| **33** | create-context-graph.dev (user's product) | **[`workshop-own-experience.md`](../workshop/workshop-own-experience.md) Slide 6** (larger greenfield) — add as a demo option |
...
| **40** | Resources list with hyperlinks | **[`workshop-resources.md`](../workshop/workshop-resources.md)** — cross-reference and merge |
...
### To [`vibecode-keynote-v3.md`](../keynote/vibecode-keynote-v3.md) (the keynote — separate from workshop)
...
- [ ] Use the full CE Resources list to expand [`workshop-resources.md`](../workshop/workshop-resources.md), or keep that doc tight? **Lean: add the 7 named items above, skip the rest.**
